/* Automatically generated nanopb header */
/* Generated by nanopb-0.4.9.1 */

#ifndef PB_CONTROL_BOARD_PB_H_INCLUDED
#define PB_CONTROL_BOARD_PB_H_INCLUDED
#include "utils/pb.h"
#include "drive.pb.h"
#include "relays.pb.h"

#if PB_PROTO_HEADER_VERSION != 40
#error Regenerate this file with the current version of nanopb generator.
#endif

/* Struct definitions */
typedef struct _ControlBoardCommand {
    bool has_drive;
    DriveCommand drive;
    bool has_relays;
    RelaysCommand relays;
} ControlBoardCommand;

typedef struct _ControlBoardData {
    bool has_drive;
    DriveData drive;
    bool has_relays;
    RelaysData relays;
} ControlBoardData;


#ifdef __cplusplus
extern "C" {
#endif

/* Initializer values for message structs */
#define ControlBoardCommand_init_default         {false, DriveCommand_init_default, false, RelaysCommand_init_default}
#define ControlBoardData_init_default            {false, DriveData_init_default, false, RelaysData_init_default}
#define ControlBoardCommand_init_zero            {false, DriveCommand_init_zero, false, RelaysCommand_init_zero}
#define ControlBoardData_init_zero               {false, DriveData_init_zero, false, RelaysData_init_zero}

/* Field tags (for use in manual encoding/decoding) */
#define ControlBoardCommand_drive_tag            1
#define ControlBoardCommand_relays_tag           2
#define ControlBoardData_drive_tag               1
#define ControlBoardData_relays_tag              2

/* Struct field encoding specification for nanopb */
#define ControlBoardCommand_FIELDLIST(X, a) \
X(a, STATIC,   OPTIONAL, MESSAGE,  drive,             1) \
X(a, STATIC,   OPTIONAL, MESSAGE,  relays,            2)
#define ControlBoardCommand_CALLBACK NULL
#define ControlBoardCommand_DEFAULT NULL
#define ControlBoardCommand_drive_MSGTYPE DriveCommand
#define ControlBoardCommand_relays_MSGTYPE RelaysCommand

#define ControlBoardData_FIELDLIST(X, a) \
X(a, STATIC,   OPTIONAL, MESSAGE,  drive,             1) \
X(a, STATIC,   OPTIONAL, MESSAGE,  relays,            2)
#define ControlBoardData_CALLBACK NULL
#define ControlBoardData_DEFAULT NULL
#define ControlBoardData_drive_MSGTYPE DriveData
#define ControlBoardData_relays_MSGTYPE RelaysData

extern const pb_msgdesc_t ControlBoardCommand_msg;
extern const pb_msgdesc_t ControlBoardData_msg;

/* Defines for backwards compatibility with code written before nanopb-0.4.0 */
#define ControlBoardCommand_fields &ControlBoardCommand_msg
#define ControlBoardData_fields &ControlBoardData_msg

/* Maximum encoded size of messages (where known) */
#define CONTROL_BOARD_PB_H_MAX_SIZE              ControlBoardData_size
#define ControlBoardCommand_size                 91
#define ControlBoardData_size                    265

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif
